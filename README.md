...
Everything lives under `unmango.*`: the life domains below, and the infrastructure APIs (`cli`, `cmd`, `protofs`, `discord/backup`) documented at the end.
...
Relationship vocabulary shared by every life domain.
...
Measurement vocabulary shared by every life domain.
...
Together, `ref` and `uom` are the only packages a life domain imports.
...
Private key material is deliberately absent, the same reason commit bytes are absent from `vcs.commit` and generated bytes are absent from `codegen.artifact`: `material_uri` is a `protofs` root, and reading or provisioning the actual PEM bytes is an ordinary filesystem operation this API does not restate.
...
These four packages sit under `unmango.*` alongside the life domains, and share none of their conventions: they define services, and nothing in them is a resource with an identity band.

### cli

[`proto/unmango/cli`](./proto/unmango/cli)

Parses a command line into a concrete syntax tree, and a typed spec into a utility invocation.

- `ParserService`: `ParseCommand`, raw string to a `CommandLine` CST
- `AnalysisService`: `Parse`, a packed spec message to a `Utility`
- `Convention`, `Option`, `Utility`: the flag conventions a parse is read under

### cmd

[`proto/unmango/cmd`](./proto/unmango/cmd)

Builds and runs a process.

- `BuilderService`: `FromJson`, `FromYaml`, `FromProto`, a spec to a `Process`
- `ExecutionService`: `Run`, a `Process` to its output and exit code

`v1alpha2` is canonical.
`v1alpha1` is deprecated in its favor: its `Run` took a `cli` `Utility` where this one takes a `Process`, and `BuilderService` is what converts between them.

### protofs

[`proto/unmango/protofs`](./proto/unmango/protofs)
...
[`proto/unmango/discord/backup`](./proto/unmango/discord/backup)
